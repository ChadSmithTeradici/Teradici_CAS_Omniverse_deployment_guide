...
As it pertains to Omniverse, it generally is installed on the host system(s) that themselves are connected to a local area networks (LANs) for optimal real-time in application collaboration. The Necleus server does have build-in file synch and share service, but its not recommended for real-time collaboration between participates remotely. The illustration below shows each of the deployment senerios that will be discussed:
