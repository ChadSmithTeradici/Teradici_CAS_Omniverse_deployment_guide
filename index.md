...
As it pertains to **Omniverse** in these deployment scenarios, it generally is installed on the host system(s) that are usually on a local area networks (LANs) with the other participating workstations in the same LAN for optimal real-time in application collaboration. Remote contributers will remote connect into a workstation that has all applications, including Omniverse installed on that machine. Files can either be kept local or centralized on NAS (network attached storage) with the Necleus server acting a service syning files changes for all participants. The Necleus server does have build-in file synch and share service, but doesn't allow for real-time collaboration between participates remotly, which diminishes the value of Omniverse. The illustration below shows each of the deployment senerios that will be discussed:
